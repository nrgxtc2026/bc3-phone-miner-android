...
/*
 * BC3 hashes exactly two interleaved 80-byte headers, then hashes each
 * 32-byte digest twice more.  All three messages fit in one SHA3-256 rate
 * block, so the generic streaming context and its buffer copies are needless.
 */
void sha3d256_2x64_fixed( void *dst, const void *data )
{
   keccak64_ctx_v128 ctx;
   keccak64_ctx_v128 *kc = &ctx;
   const v128_t *in = (const v128_t *)data;
   v128_t digest[4];
   int i;
   ...
   keccak64x2_init( kc, 256 );
   for ( i = 0; i < 10; i++ )
      kc->w[i] = v128_xor( kc->w[i], in[i] );
   kc->w[10] = v128_xor( kc->w[10], v128_64( 0x06 ) );
   kc->w[16] = v128_xor( kc->w[16],
                                  v128_64( 0x8000000000000000ULL ) );
   KECCAK_F_1600;
   NOT64( kc->w[ 1], kc->w[ 1] );
   NOT64( kc->w[ 2], kc->w[ 2] );
   NOT64( kc->w[ 8], kc->w[ 8] );
   ...
   NOT64( kc->w[17], kc->w[17] );
   NOT64( kc->w[20], kc->w[20] );
   v128_memcpy( digest, kc->w, 4 );

   /* SHA3-256(digest[32]), twice. */
   for ( int round = 0; round < 2; round++ )
   {
      keccak64x2_init( kc, 256 );
      for ( i = 0; i < 4; i++ )
         kc->w[i] = v128_xor( kc->w[i], digest[i] );
      kc->w[4] = v128_xor( kc->w[4], v128_64( 0x06 ) );
      kc->w[16] = v128_xor( kc->w[16],
                                     v128_64( 0x8000000000000000ULL ) );
      KECCAK_F_1600;
      NOT64( kc->w[ 1], kc->w[ 1] );
